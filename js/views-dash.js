/* ============ RACHA · Vistas: Inicio, Dashboard, Trades, Calendario, Cuentas, Metas ============ */
...
(() => {
  ...
  function tradeTable(trades, paginate) {
    if (!trades.length) return UI.empty('candles', 'Sin trades', 'Agrega tu primera operación con el botón +.');
    ...
    if (paginate) {
      ...
      pager = `<div class="pager">
        ...
        <div class="pg-nav">
          ...
          <span class="small">Página ${page} de ${total}</span>
    ...
    return `<div class="table-wrap"><table class="trades">
      <thead><tr><th>Fecha / Hora</th><th>Símbolo</th><th>Cuenta</th><th>Lado</th><th>Qty</th><th>Entrada</th><th>Salida</th><th>P&amp;L</th><th>%</th><th>Duración</th><th>Estado</th></tr></thead>
      <tbody>${body}</tbody></table></div>${pager}`;
  }
  V.tradeTable = tradeTable;

  // -------- calendario con columna semanal (estilo TradeSyncer) --------
  function calendarFull(key) {
    ...
    let html = `<div class="ts-cal">
      <div class="ts-cal-head">${UI.DOW.map(d => `<span>${d}</span>`).join('')}<span>Semana</span></div>
  ...
  // -------- INICIO se define en views-inicio.js (estilo Home de TradeSyncer) --------
  ...
  V.dashboard = function () {
    ...
    const top = `<div class="grid2-wide">
      ...
      <div class="card big-kpi">
        <div><div class="bk-l">Profit Factor</div><div class="bk-v up">${st.profitFactor === Infinity ? '∞' : st.profitFactor}</div><div class="muted small">${st.profitFactor >= 1 ? 'Rentable' : 'En rojo'}</div></div>
    ...
    const charts = `<div class="grid4">
      ...
      <div class="card chartcard"><div class="cc-l">PNL acumulado</div>${UI.areaChart(cum, '#22c55e')}<div class="muted small">${UI.dateUS('2026-06-21')} → ${UI.dateUS('2026-06-22')}</div></div>
      <div class="card chartcard"><div class="cc-l">Drawdown</div><svg class="spark" viewBox="0 0 300 90" preserveAspectRatio="none"><line x1="4" y1="20" x2="296" y2="20" stroke="#ef4444" stroke-width="2" stroke-dasharray="0"/></svg><div class="muted small">Actual: <span class="up">$0.00</span></div></div>
      <div class="card chartcard"><div class="cc-l">PNL por día</div>${UI.barChart(Q.pnlByDay(key))}<div class="muted small">${UI.monthLabel(key)}</div></div>
    </div>`;

    const cards3 = `<div class="grid3">
      ${tradeStat('Best Trade', best, 'up')}
      <div class="card statcard"><div class="sc-l">Total Trades</div><div class="sc-v">${st.n}</div><div class="muted small">operaciones</div></div>
      ${tradeStat('Worst Trade', worst, 'down')}
    </div>`;

    const hist = `<div class="card"><div class="card-head"><div class="ch-t">${UI.icon('list', '', 18)} Trades History</div><button class="link" data-act="go" data-route="trades">Ver todos</button></div>${tradeTable(Q.tradesDesc().slice(0, 10), false)}</div>`;
  ...
  V.trades = function () {
    ...
    const cards = `<div class="grid4">
      ...
      ${kpi('Profit Factor', st.profitFactor === Infinity ? '∞' : st.profitFactor, 'Rentable', 'up')}
  ...
  V.calendario = function () {
    ...
    const setupCard = setups.length ? `<div class="card"><div class="card-head"><div class="ch-t">${UI.icon('bolt', '', 18)} Por setup</div></div>
    ...
    return `<div class="page">${calCard(key)}
      <div class="grid4">
        ${kpi('Neto del mes', UI.bigK(st.net), st.n + ' trades', UI.pnlClass(st.net))}
        ${kpi('Win rate', st.winRate + '%', '')}
        ${kpi('Profit factor', st.profitFactor === Infinity ? '∞' : st.profitFactor, '', 'up')}
        ${kpi('Expectativa', UI.money(st.expectancy, true), 'por trade', UI.pnlClass(st.expectancy))}
      </div>${setupCard}</div>`;
  };
  ...
  V.cuentas = function () {
    ...
    const accCard = `<div class="card"><div class="card-head"><div class="ch-t">${UI.icon('building', '', 18)} Cuentas de fondeo</div><button class="link" data-act="addAccount">+ Cuenta</button></div>
      <div class="table-wrap"><table class="trades acc-table"><thead><tr><th>Cuenta</th><th>Firma</th><th>Fase</th><th>Tamaño</th><th>P&amp;L</th><th>Balance</th><th>Estado</th></tr></thead><tbody>
    ...
    const payCard = `<div class="card"><div class="card-head"><div class="ch-t">${UI.icon('coin', '', 18)} Payouts</div><button class="link" data-act="addPayout">+ Payout</button></div>
      <div class="payhero"><div><div class="ph-k">Total cobrado</div><div class="ph-v up">${UI.usd(Q.payoutsTotal())}</div></div><div class="ph-tag">${Q.payoutsCount()} cobros · prom ${UI.usd(Q.avgPayout())}</div></div>
  ...
  // -------- CARTERA (gastos del mes + rentabilidad, personalizable) --------
  V.cartera = function () {
    ...
    const summary = `<div class="card snow-card glass">
      <div class="snow-card-l">
        <div class="eyebrow">${UI.icon('wallet', '', 15)} TU CARTERA</div>
        <h1>Lo que entra, lo que sale y lo que crece.</h1>
        <p class="muted">Tu ingreso del trading menos tus gastos del mes. Lo que queda libre alimenta tu Snowball.</p>
        <div class="cartera-flow">
          ...
          <span class="cf-op">=</span>
          <div class="cf free"><span class="muted small">Libre para invertir</span><b class="ice">${UI.usd(free)}</b><span class="muted small">${rate}% de tu ingreso</span></div>
        </div>
      </div>
    ...
    const rentab = `<div class="card">
      <div class="card-head"><div class="ch-t">${UI.icon('trendUp', '', 18)} Tu rentabilidad</div><button class="link" data-act="go" data-route="dashboard">Ver journal</button></div>
      <div class="grid4">
        ${kpi('Capital generado', UI.usd(Q.payoutsTotal()), Q.payoutsCount() + ' payouts', 'up')}
        ${kpi('PNL histórico', UI.bigK(st.net), st.n + ' trades', UI.pnlClass(st.net))}
        ${kpi('Win rate', st.winRate + '%', '')}
        ${kpi('Profit factor', st.profitFactor === Infinity ? '∞' : st.profitFactor, '', 'up')}
      </div>
    </div>`;
  ...
  V.daySheet = function (date) {
    ...
    return `<div class="sheet-head"><div class="h2">${UI.dateLong(date)}</div><div class="day-net ${UI.pnlClass(net)}">${UI.pnl(net)} <span class="muted small">· ${st.wins}W ${st.losses}L</span></div></div>
      ${ts.length ? tradeTable(ts, false) : UI.empty('candles', 'Sin trades este día')}
      <button class="btn btn-primary full mt8" data-act="addTradeFor" data-date="${date}">${UI.icon('plus', '', 16)} Agregar trade de este día</button>`;
  };
  ...
  V.accountSheet = function (id) {
    ...
    return `<div class="sheet-head"><div class="h2">${UI.dot(f.color)} ${UI.esc(a.alias)}</div><div class="muted small">${f.label} · ${UI.usd(a.size)} · <span style="color:${stt.color}">${stt.label}</span></div></div>
      <div class="grid2 mb12"><div class="card statbox"><div class="sb-val">${UI.usd(Q.accBalance(a))}</div><div class="sb-lbl">Balance estimado</div></div><div class="card statbox"><div class="sb-val ${UI.pnlClass(p)}">${UI.money(p, true)}</div><div class="sb-lbl">P&amp;L</div></div></div>
      ...
      <div class="muted small mb6">Últimos trades</div>${tradeTable(Q.accTrades(a.id).slice().sort((x, y) => (x.date < y.date ? 1 : -1)).slice(0, 8), false)}
      <div class="btn-row mt12"><button class="btn btn-ghost" data-act="editAccount" data-id="${a.id}">${UI.icon('edit', '', 15)} Editar</button><button class="btn btn-danger" data-act="delAccount" data-id="${a.id}">Eliminar</button></div>`;
  };
})();
