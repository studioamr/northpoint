...
(() => {
  ...
  V.academia = function () {
    ...
    const hero = `<div class="card course-hero">
      <div class="ch-l">
        <div class="eyebrow">${UI.icon('academy', '', 15)} CURSO · DE CERO A PAYOUT</div>
        <h1>Aprende el sistema, luego llévalo a tu journal.</h1>
        <p class="muted">${Data.COURSE.length} módulos · ${Data.allLessons().length} lecciones. Tu progreso se guarda solo.</p>
        <div class="course-prog">
          <div class="cp-bar">${UI.bar(cs.done, cs.total, 'var(--brand)')}</div>
          <span class="cp-txt"><b>${cs.pct}%</b> · ${cs.done}/${cs.total} lecciones</span>
        </div>
        <button class="btn btn-primary" data-act="continueCourse">${UI.icon('play', '', 16)} ${cs.done ? 'Continuar curso' : 'Empezar curso'}${next ? ` · ${UI.esc(next.title)}` : ''}</button>
      </div>
      <div class="ch-r">${UI.donut(cs.pct, 'var(--brand)', cs.pct + '%')}<span class="muted small">completado</span></div>
    </div>`;
    ...
    const modules = Data.COURSE.map(m => {
      ...
      return `<div class="card module">
        <div class="mod-head">
          ...
          <div class="mod-info"><div class="mod-t"><span class="mod-n">Módulo ${m.n}</span> · ${UI.esc(m.title)}</div><div class="muted small">${UI.esc(m.desc)}</div></div>
  ...
  V.lesson = function () {
    ...
    return `<div class="page lesson-page">
      <button class="link mb12" data-act="go" data-route="academia">${UI.icon('back', '', 15)} Volver a la Academia</button>
      <div class="lesson-grid">
        <div class="lesson-main">
          <div class="video">
            <div class="video-ph">${UI.icon('playc', '', 56)}<span>Video de la lección</span><small class="muted">Pega aquí tu video (YouTube/Vimeo) al publicar</small></div>
          </div>
          <div class="lesson-meta">
            <span class="pill-mod" style="color:${mod.color};border-color:${mod.color}55">Módulo ${mod.n}</span>
          ...
          <div class="lesson-actions">
            <button class="btn ${done ? 'btn-ghost' : 'btn-primary'}" data-act="toggleLesson" data-id="${l.id}">${done ? UI.icon('checkc', '', 16) + ' Completada' : UI.icon('check', '', 16) + ' Marcar completada'}</button>
            <button class="btn btn-ghost" data-act="nextLesson">Siguiente ${UI.icon('chevR', '', 16)}</button>
          </div>
        </div>
